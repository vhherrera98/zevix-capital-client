...
import { useGetAllDocumentsRequestsQuery } from "@/lib/Redux/web/endpoints/documents/persons";
...
export default function PageRequestsDocuments() {

  const router = useRouter();
  const session = useSession();

  const { data, isLoading, isError } = useGetAllDocumentsRequestsQuery();
  const columns: ColumnDef<Documento>[] = [
    {
      accessorFn: (row) => row.persona.nombres,
      accessorKey: "nombres",
      header: ({ column }) => <DataTableColumnHeader column={column} title={"Nombre(s)"} />,
      cell: ({ row }) => <ContainerCell>{row.getValue("nombres")}</ContainerCell>,
      meta: { label: "Nombre(s)" }
    },
    {
      accessorFn: (row) => row.persona.apellidos,
      accessorKey: "apellidos",
      header: ({ column }) => <DataTableColumnHeader column={column} title={"Apellido(s)"} />,
      cell: ({ row }) => <ContainerCell>{row.getValue("apellidos")}</ContainerCell>,
      meta: { label: "Apellido(s)" }
    },
    {
  ...
  const loading = isLoading || !session || !session?.data;
  ...
  return (
    <div className="grid grid-cols-1 gap-5">
      <div className='flex flex-row items-center justify-between'>
        <h1 className="text-2xl font-bold">Documentos (Solicitudes)</h1>
        <Button
          type="button"
          className="btn-primary"
          onClick={() => router.push("/dashboard/documents/requests/beneficiaries")}
        >
          Beneficiarioss
        </Button>
      </div>
